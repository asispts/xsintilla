...
typedef struct {
    ...
    // invoked.
    void (*redrawWithRectsFunction)(REALcontrolInstance,
                                    REALobject context,
...
#define REALdontEraseBackground \
    (1 << 8U) // Set this flag if you don't want REALbasic to automatically erase the control's background.
              // Note, this only affects Windows, use it to reduce flicker if your control already handles background
              // painting.
#define REALcontrolHandlesKeyboardNavigation \
    (1 << 10) // This flag is only used on Windows, use it to suppress the default keyboard navigation.
// For example, pressing the arrow keys or pressing a keyboard mnemonic moves focus to another control,
// but this is not always desirable, especially when writing your own edit control.
#define REALcontrolHandlesTabKey \
    (1 << 11) // This flag specifies whether or not the control handles the tab key, especially useful if
...
typedef struct {
    ...
    REALdbCursor (*getFieldSchemaCursor)(
        dbDatabase*,
        REALstring); // dbCursor *(*fieldCursor)(dbDatabase *, REALstring)) /* optional */

    REALdbCursor (*directSQLSelect)(
        dbDatabase*,
        REALstring); // DatabaseCursorObject (*directSQLSelect)(dbDatabase *, REALstring selectString);
    void (*directSQLExecute)(dbDatabase*,
                             REALstring); // void (*directSQLExecute)(dbDatabase *, REALstring executeString);

    void (*createTable)(dbDatabase*, REALstring, REALnewColumn*, unsigned char*, int); // void (*createTable)(dbDatabase
                                                                                       // *, REALstring name,
                                                                                       // REALnewColumn *columns,
                                                                                       // unsigned char *primaryKey, int
                                                                                       // primaryKeyCount);
    void (*addTableRecord)(
        dbDatabase*,
        REALstring,
        REALcolumnValue*); // void (*addTableRecord)(dbDatabase *, REALstring tableName, REALcolumnValue *values);
    ...
    void (*alterColumnType)(dbDatabase* db,
    ...
    void (*alterColumnConstraint)(dbDatabase* db,
                                  REALstring tableName,
                                  REALstring columnName,
                                  REALstring whichConstraint,
                                  REALstring oldConstraint,
                                  REALstring newConstraint);
...
typedef struct {
    ...
    REALstring (*cursorColumnName)(
        dbCursor*,
        int column);                  // REALstring (*cursorColumnName)(dbCursor *, int column); /* optional */
    ...
    void (*cursorColumnValue)(
        ...
        void**,
        unsigned char*,
        int*); // void (*cursorColumnValue)(dbCursor *, int column, void **value, dbFieldType *type, int *length);
...
enum {
    ...
    pictureGDPtr, // Console Only: Pointer to a copy of GD data, which is 11 bytes meta data then pixels follow in ARGB
                  // format
